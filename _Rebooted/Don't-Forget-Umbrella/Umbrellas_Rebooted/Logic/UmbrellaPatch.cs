...
namespace Umbrellas_Rebooted.Logic
{
	public class UmbrellaPatch
	{
		...
		public static bool drawHoverTextPrefix(IClickableMenu __instance, SpriteBatch b, StringBuilder text, SpriteFont font, int xOffset, int yOffset, int moneyAmountToDisplayAtBottom, string boldTitleText, int healAmountToDisplay, string[] buffIconsToDisplay, ref Item hoveredItem, int currencySymbol, int extraItemToShowIndex, int extraItemToShowAmount, int overrideX, int overrideY, float alpha, CraftingRecipe craftingIngredients, IList<Item> additional_craft_materials)
		{
			try
			{
				if (hoveredItem is not null)
				{
					...
					{
						if (ModEntry.bestHatNames.Contains(hoveredItem.Name) || 
							ModEntry.customBestHats.Contains(hoveredItem.Name))
						{
							words = string.Join(' ', ModEntry.Instance.Helper.Translation.Get("ui.water-proof"),
													 ModEntry.Instance.Helper.Translation.Get("ui.proof-level.major")
											   ).Split(' ');
						}

						else if (ModEntry.goodHatNames.Contains(hoveredItem.Name) || 
								 ModEntry.customGoodHats.Contains(hoveredItem.Name))
						{
							words = string.Join(' ', ModEntry.Instance.Helper.Translation.Get("ui.water-proof"),
													 ModEntry.Instance.Helper.Translation.Get("ui.proof-level.moderate")
											   ).Split(' ');
						}

						else if (ModEntry.shirtsNames.Contains(hoveredItem.Name))
						{
							words = string.Join(' ', ModEntry.Instance.Helper.Translation.Get("ui.water-proof"),
													 ModEntry.Instance.Helper.Translation.Get("ui.proof-level.slight")
											   ).Split(' ');
						}
					}
		...
		/// <summary>
		/// I should probably remove most of the below code as it's not necessary for weapon tool-tips, but I don't want to risk bugs.
		/// </summary>
			...
			try
			{
				if (hoveredItem is object)
				{
					if (ModEntry.umbrellaNames.Contains(hoveredItem.Name))
					{
						...
						int width = Math.Max(healAmountToDisplay != -1 ? (int)font.MeasureString(healAmountToDisplay + ModEntry.Instance.Helper.Translation.Get("ui.add-energy") + 32).X : 0, Math.Max((int)font.MeasureString(text).X, boldTitleText != null ? (int)Game1.dialogueFont.MeasureString(boldTitleText).X : 0)) + 32;
						...
						if (hoveredItem != null)
						{
							...
							if (buffIconsToDisplay != null)
							{
								for (int j = 0; j < buffIconsToDisplay.Length; j++)
								{
									...
									{
										width = (int)Math.Max(width, font.MeasureString(Game1.content.LoadString("Strings\\UI:ItemHover_Buff" + j, maxStat)).X + buffer);
									}
								}
							}
						}
						...
						IClickableMenu.drawTextureBox(b, Game1.menuTexture, new Rectangle(0, 256, 60, 60), x, y4, width + (craftingIngredients != null ? 21 : 0), height2, Color.White * alpha);
						if (boldTitleText != null)
						{
							Vector2 bold_text_size = Game1.dialogueFont.MeasureString(boldTitleText);
							IClickableMenu.drawTextureBox(b, Game1.menuTexture, new Rectangle(0, 256, 60, 60), x, y4, width + (craftingIngredients != null ? 21 : 0), (int)Game1.dialogueFont.MeasureString(boldTitleText).Y + 32 + (int)(hoveredItem != null && categoryName.Length > 0 ? font.MeasureString("asd").Y : 0f) - 4, Color.White * alpha, 1f, drawShadow: false);
							...
							{
								Utility.drawTextWithShadow(b, bold_title_subtext, Game1.smallFont, new Vector2(x + 16 + bold_text_size.X, (int)(y4 + 16 + 4 + bold_text_size.Y / 2f - small_text_size.Y / 2f)), Game1.textColor);
							}
						...
						if (hoveredItem != null && categoryName.Length > 0)
						{
							...
							if (hoveredItem is Tool && (hoveredItem as Tool).GetTotalForgeLevels() > 0)
							{
								...
								{
									Utility.drawTextWithShadow(b, " (" + forges + "/" + (hoveredItem as Tool).GetMaxForges() + ")", font, new Vector2(x + 16 + font.MeasureString(forged_string2).X, y4 + 16 + 4), Color.DimGray, 1f, -1f, 2, 2);
								}
							...
							if (hoveredItem is MeleeWeapon && (hoveredItem as MeleeWeapon).GetEnchantmentLevel<GalaxySoulEnchantment>() > 0)
							{
								...
								{
									Utility.drawTextWithShadow(b, " (" + level + "/" + enchantment.GetMaximumLevel() + ")", font, new Vector2(x + 16 + font.MeasureString(forged_string).X, y4 + 16 + 4), Color.DimGray, 1f, -1f, 2, 2);
								}
								y4 += (int)font.MeasureString("T").Y;
							}
						}
						...
						if (moneyAmountToDisplayAtBottom > -1)
						{
							...
							{
								case 0:
									b.Draw(Game1.debrisSpriteSheet, new Vector2(x + 16 + font.MeasureString(string.Concat(moneyAmountToDisplayAtBottom)).X + 20f, y4 + 16 + 16), Game1.getSourceRectForStandardTileSheet(Game1.debrisSpriteSheet, 8, 16, 16), Color.White, 0f, new Vector2(8f, 8f), 4f, SpriteEffects.None, 0.95f);
									break;
								case 1:
									b.Draw(Game1.mouseCursors, new Vector2(x + 8 + font.MeasureString(string.Concat(moneyAmountToDisplayAtBottom)).X + 20f, y4 + 16 - 5), new Rectangle(338, 400, 8, 8), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
									break;
								case 2:
									b.Draw(Game1.mouseCursors, new Vector2(x + 8 + font.MeasureString(string.Concat(moneyAmountToDisplayAtBottom)).X + 20f, y4 + 16 - 7), new Rectangle(211, 373, 9, 10), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
									break;
								case 4:
									b.Draw(Game1.objectSpriteSheet, new Vector2(x + 8 + font.MeasureString(string.Concat(moneyAmountToDisplayAtBottom)).X + 20f, y4 + 16 - 7), Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, 858, 16, 16), Color.White, 0f, Vector2.Zero, 4f, SpriteEffects.None, 1f);
									break;
							}
							y4 += 48;
						}
						if (extraItemToShowIndex != -1)
						{
							...
							IClickableMenu.drawTextureBox(b, Game1.menuTexture, new Rectangle(0, 256, 60, 60), x, y4 + 4, width + (craftingIngredients != null ? 21 : 0), (int)minimum_box_height, Color.White);
							y4 += 20;
